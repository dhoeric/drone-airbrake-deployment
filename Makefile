.PHONY: test drone-airbrake-deployment build fmt vet errcheck lint install update release-dirs release-build release-copy release-check release coverage

DIST := dist
EXECUTABLE := drone-airbrake-deployment
GO ?= go

# for dockerhub
DEPLOY_ACCOUNT := dhoeric
